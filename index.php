...
{
    global $curr_dir;
    $full_output = $curr_dir . "/" . $output;
    $full_input = $curr_dir . "/" . $input;
    clearstatcache();
    ...
    for ($i = 2; $i < count($files); $i++) {
        ...
        if ($ext == "docx" || $ext == "doc") {
            $converter = new OfficeConverter("$full_input/$files[$i]");
            $converter->convertTo($filename . '.pdf');
            rename("$full_input/$filename" . '.pdf', $full_output . '/' . $filename . '.pdf');
...
try {
    ...
    docx2pdf($inputFolder, $outputFolder, $cb_show);
} catch (Exception $e) {
    echo ($e->getMessage());
}
